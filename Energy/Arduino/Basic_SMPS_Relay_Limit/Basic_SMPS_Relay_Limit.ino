...
boolean CL_mode = 1;
boolean slowLoopTrigger = 0;

float u0iL, deltaiL, u0vO, deltavO, negative_cond, incremental_cond; // MPPT variable
...
void setup() {
  ...
  pinMode(8, OUTPUT); // This pin outputs high/low to the relay going to the battery
  pinMode(13, OUTPUT);  //Pin13`/ Pin 2 is the input from the CL/OL switch
  analogReference(EXTERNAL); // We are using an external analogue reference for the ADC

  pinMode(4, OUTPUT);  //This pin will time the slow loop
  ...
  // TimerB2 initilaisation for slow loop interrupt
  TCB1.CCMP = 0x7fff; // With 2 prescaler ~244.1 Hz frequency
 ...
 void loop() {
  if(loopTrigger) { // This loop is triggered, it wont run unless there is an interrupt
    ...
    Boost_mode = 0; // input from the Buck_Boost switch
  // The code above checks the outputs to the battery and switches the relay accordingly
    ...
    Serial.println("Vpd = " + String(vpd, 3) + "V");
    Serial.println("IL = " + String(iL, 3) + "A");
    Serial.println("Duty ref = " + String(dutyref, 3));
    Serial.println("Boost_mode= " + String(Boost_mode, 3));
    Serial.println("CL_mode = " + String(CL_mode, 3));
    Serial.println("FAST_LOOP");*/
  }
  ...
  if(slowLoopTrigger){
    ...
    if (Boost_mode){
      ...
      if (va > high_voltage_limit || vb < low_voltage_limit || iL > current_limit){
          digitalWrite(8, LOW);
        }
      else{
          digitalWrite(8, HIGH);
    ...
    incremental_cond = deltaiL/deltavO;
    // Idealy we want it such that V = -IdV/dI
    
   // vref = -iL*(deltavO/deltaiL);
    
    
   
    if(incremental_cond < negative_cond){
      // decrease voltage
      // positive gradient threshold has to be more sensitive than negative
      // as the power fall-off for high voltages is far steeper than the
      // power ramp up for lower voltages
      vref = vref - 0.1 ; 
      }
    else if(incremental_cond > negative_cond){
        //increase voltage
        vref = vref + 0.1 ;
      }
    ...
    Serial.println("Vb = " + String(vb, 3) + "V");
    Serial.println("Vpd = " + String(vpd, 3) + "V");
    Serial.println("Delta vO = " + String(deltavO, 3) + "V");
    Serial.println("Delta iL = " + String(deltaiL, 3) + "A");
    u0iL = iL;
    u0vO = vb;
    Serial.println("SLOW_LOOP");
    slowLoopTrigger = 0;
    }
