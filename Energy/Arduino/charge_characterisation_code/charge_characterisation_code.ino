...
float kpp=0.08,kip=0,k2ip=-30; // power pid.
float u0p,u1p,delta_up,e0p,e1p,e2p; // Internal values for the power controller

float high_voltage_limit = 5.15;
float low_voltage_limit = 4.6; // Voltage limits
...
void setup() {
  ...
  if (SD.exists("BC2.csv")) { // Wipe the datalog when starting
    SD.remove("BC2.csv");
  }
  ...
  pinMode(8, OUTPUT);
  pinMode(5, OUTPUT); // Relay outputs
  digitalWrite(5, HIGH); 
 ...
 void loop() {
  if(loopTrigger) { // This loop is triggered, it wont run unless there is an interrupt
    ...
    if (Boost_mode){
      if (CL_mode) { //Closed Loop Boost
          current_limit = 2.05;
          ev = vref - va;  //voltage error at this time
      ...
      if (CL_mode) { // Closed Loop Buck
          ...
          // current
          current_limit = 2.1; // Buck has a higher current limit
          ev = vref - vb;  //voltage error at this time
          cv=pidv(ev);  //voltage pid
          cv=saturation(cv, current_limit, 0); //current demand saturation
          ei=cv-iL; //current error
          closed_loop=pidi(ei);  //current pid
          closed_loop=saturation(closed_loop,0.995,0.005);  //duty_cycle saturation
          pwm_modulate(closed_loop); //pwm modulation
      }else{ // Open Loop Buck
          current_limit = 2.05; // Buck has a higher current limit
  ...
  if(loopCounter == 500){
      ...
      if (va > high_voltage_limit || va < low_voltage_limit || iL > current_limit){
        digitalWrite(8, HIGH);
      }
      else{
        digitalWrite(8, LOW);
    ...
    // vref = -iL*(deltavO/deltaiL);
      ...
      File dataFile = SD.open("BC2.csv", FILE_WRITE); // open our CSV file
...
float pidp(float pid_input){
  ...
  e_integration_2=e0p;
  //anti-windup
  if(u1p >= high_voltage_limit){
    e_integration = 0;
    e_integration_2 = 0;
  } else if (u1p <= low_voltage_limit) {
    e_integration = 0;
    e_integration_2 = 0;
  }
  
  delta_up = k2ip*(Ts*Ts)*e_integration_2 + kpp*(e0p-2*e1p+e2p); //incremental PID programming avoids integrations.
  u0p = u1p + delta_up;  //this time's control output
  
  //output limitation
  u0p = saturation(u0p,high_voltage_limit,low_voltage_limit);
