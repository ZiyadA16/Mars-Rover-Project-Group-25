...
float kpv=0.043,kiv=20.1,kdv=0; // voltage pid.
float u0v,u1v,delta_uv,e0v,e1v,e2v; // u->output; e->error; 0->this time; 1->last time; 2->last last time
float kpi=0.075,kii=39.4,kdi=0; // current pid.
float u0i,u1i,delta_ui,e0i,e1i,e2i; // Internal values for the current controller

float kpp=0.013,kip=1.88,kdp=0; // power pid.
...
void setup() {
  ...
  if (SD.exists("PVB3bA.csv")) { // Wipe the datalog when starting
    SD.remove("PVB3bA.csv");
  }
...
void sampling(){
  ...
  vb = sensorValue0 * (4.096 / 1023.0) * 2.6; // Convert the Vb sensor reading to volts
...
float pidp(float pid_input){
  ...
  delta_up = kpp*(e0i-e1i) + kip*Ts_power*e_integration + kdp/Ts_power*(e0p-2*e1p+e2p);//incremental PID programming avoids integrations.
  u0p = u1p + delta_up;  //this time's control output
  
  //output limitation
  u0p = saturation(u0p,5.15,4.6);
