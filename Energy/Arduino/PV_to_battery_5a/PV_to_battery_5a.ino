...
float kpp=0.00084,kip=0,k2ip=-450; // power pid.
...
void setup() {
  ...
  if (SD.exists("PVB4aA.csv")) { // Wipe the datalog when starting
    SD.remove("PVB4aA.csv");
  }
 ...
 void loop() {
  if(loopTrigger) { // This loop is triggered, it wont run unless there is an interrupt
    ...
    if(deltaVa == 0){
      ...
      ep = 0.4-deltaP/deltaVa; // we want dP/dV = 0 so ep is -dP/dV
    }
            
    closed_loop = pidp(ep); // power PII2 controller
  ...
  if(loopCount == loopMaxValue){
    ...
    Serial.println(dataString);
      /*File dataFile = SD.open("PVB4aA.csv", FILE_WRITE); // open our CSV file
...
float pidp(float pid_input){
  ...
  u0p = saturation(u0p,0.999, 1-(vb/4.7));
